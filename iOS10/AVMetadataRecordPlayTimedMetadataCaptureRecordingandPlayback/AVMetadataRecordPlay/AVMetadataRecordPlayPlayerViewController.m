...
@interface AVMetadataRecordPlayPlayerViewController () <AVPlayerItemMetadataOutputPushDelegate, PHPhotoLibraryChangeObserver>
...
@property (nonatomic) PHFetchResult *videoSmartAlbum;
...
- (void)awakeFromNib
{
	self.videoSmartAlbum = [PHAssetCollection fetchAssetCollectionsWithType:PHAssetCollectionTypeSmartAlbum subtype:PHAssetCollectionSubtypeSmartAlbumVideos options:nil];
...
- (void)viewDidLoad
{
	...
	[self updateLibaryButtonEnabledStatus];
}
...
- (void)updateLibaryButtonEnabledStatus
{
	if ( self.videoSmartAlbum.count > 0 ) {
		PHCollection *collection = self.videoSmartAlbum[ 0 ];
		if ([collection isKindOfClass:[PHAssetCollection class]]) {
			PHAssetCollection *assetCollection = (PHAssetCollection *)collection;
			PHFetchResult *assetsFetchResult = [PHAsset fetchAssetsInAssetCollection:assetCollection options:nil];
			self.libraryButton.enabled = ( assetsFetchResult.count > 0 );
		}
	}
	else {
...
- (void)setupPlayerForAVAsset:(AVAsset *)asset
{
...
- (void)setupPlaybackForAsset:(AVAsset *)asset
{
	dispatch_async( dispatch_get_main_queue(), ^{
		...
		[self setupPlayerForAVAsset:asset];
		self.playButton.enabled = YES;
		self.pauseButton.enabled = NO;
		[self removeAllSublayersFromLayer:self.facesLayer];
	} );
...
- (void)photoLibraryDidChange:(PHChange *)changeInstance
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
		...
		PHFetchResultChangeDetails *changeDetails = [changeInstance changeDetailsForFetchResult:self.videoSmartAlbum];
		if (changeDetails) {
			self.videoSmartAlbum = [changeDetails fetchResultAfterChanges];
			[self updateLibaryButtonEnabledStatus];
		}
    });
}
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    if ([segue.identifier isEqualToString:@"showVideos"]) {
        ...
        AVMetadataRecordPlayAssetGridViewController *assetGridViewController = segue.destinationViewController;
